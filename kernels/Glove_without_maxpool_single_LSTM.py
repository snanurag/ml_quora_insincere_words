...
def train_pred(model, train_X, train_y, val_X, val_y, n_repeat=5, callback=None):
    trn = pd.DataFrame()
    val = pd.DataFrame()
    for e in range(n_repeat):
        history = model.fit(train_X, train_y, batch_size=512, epochs=4, validation_data=(val_X, val_y), callbacks = callback, verbose=0)
        ...
        print("Repeat: ", e, "-    Val F1 Score: {:.4f}".format(best_score))
...
splits = list(StratifiedKFold(n_splits=2, shuffle=True, random_state=DATA_SPLIT_SEED).split(train_X, train_y))
for idx, (train_idx, valid_idx) in enumerate(splits):
        ...
        pred_val_y, pred_test_y, best_score = train_pred(model, X_train, y_train, X_val, y_val)
